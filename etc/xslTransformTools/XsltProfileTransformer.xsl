...
<!-- To correct the error on the Seg group name Timing in the profile  -->
<xsl:template match="HL7v2xStaticDef//SegGroup[@Name='TIIMING'and @LongName='TIIMING']">
	...
	<!-- To correct the error in the profile segGroup name .change all the name with dot with their parent Name -->
	<xsl:template
	...
	<!-- To correct the data type which is different in IHE database from HL7 database  -->
<!-- XON data type -->
	<xsl:template
	...
	<!-- Change CNS to CNN because CNS does not exist in IHE HL7 Database -->
	<xsl:template
